...
**A first real-provider version of that validation now exists as a separate project: [ModelBench](https://github.com/pelumiibiks-cell/ModelBench).** It runs two real Gemini tiers against 24 self-authored coding tasks, grading each one deterministically against a hidden pytest file rather than through `MockProvider`. First result: on the tasks the pricier tier (`gemini-flash`) actually got to run, its pass rate was statistically indistinguishable from the cheap tier (`gemini-flash-lite`) at roughly 5x the cost per passing task, directionally consistent with this router's own thesis (never pay for a stronger model than the task needs), now backed by real generations rather than only `MockProvider`. Not yet a validation of *this* router's own utility function specifically, since ModelBench doesn't call `/route`. See its README for exactly what is and isn't covered yet.
